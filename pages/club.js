...
const FAQS = [
  { q: "¿Quién decide la puntuación de Católicum?", a: "Nuestro equipo analiza cada libro basándose en fuentes públicas: el Catecismo de la Iglesia Católica, encíclicas papales y la tradición teológica. No somos una institución eclesiástica — somos católicos que ayudan a otros católicos a leer con criterio." },
  { q: "¿Es lo mismo que el Índice de libros prohibidos?", a: "No. Ese índice fue abolido en 1966. Nosotros no prohibimos nada — simplemente analizamos y puntuamos para que cada lector tome su propia decisión con más información." },
  { q: "¿Por qué dar una nota a un libro como Harry Potter?", a: "Porque muchos católicos se lo preguntan. La polémica existe, y preferimos dar una respuesta documentada a dejar el debate sin resolver. Nuestra puntuación es una opinión informada, no un dogma." },
  { q: "¿Puedo estar en desacuerdo con vuestra puntuación?", a: "Por supuesto — y para eso existe el sistema de valoraciones de usuario. Tu puntuación y comentario son tan válidos como el nuestro. El debate es la gracia del club." },
...
export default function Club() {
  ...
  const [stats, setStats] = useState({ libros: 0, valoraciones: 0 });
        ...
        <div style={{ flex: 1, minWidth: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ background: "#1F3A5F", borderBottom: "0.5px solid #2A4E7F", padding: isMobile ? "1.25rem 1.25rem 1rem" : "1.5rem 2rem 1.25rem", textAlign: "center" }}>
            ...
            <div style={{ borderTop: "0.5px solid #2A4E7F", borderBottom: "0.5px solid #2A4E7F", padding: ".3rem 0", maxWidth: 340, margin: "0 auto" }}>
              {!session ? (
                <button onClick={signInWithGoogle} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "#8AAFD4", fontFamily: "DM Sans, sans-serif", display: "flex", alignItems: "center", gap: 6, margin: "0 auto" }}>
          ...
          <div style={{ maxWidth: 680, margin: "0 auto", width: "100%", padding: isMobile ? "1.5rem 1rem" : "2.5rem 1.5rem" }}>
            ...
            <div style={{ marginBottom: "3rem" }}>
              ...
              <p style={{ fontSize: 14, color: "#6E6E73", lineHeight: 1.7, marginBottom: "1.25rem" }}>Cada libro recibe una puntuación del 1 al 10 según su alineación con la doctrina católica. No es una condena — es una orientación para que el lector decida con más información.</p>
                ...
                {RATINGS.map(function(r) {
                  return (
                    <div key={r.rango} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", background: r.bg, borderRadius: 10 }}>
                      <div style={{ fontFamily: "'EB Garamond', Georgia, serif", fontSize: 20, fontWeight: 500, color: r.color, lineHeight: 1, minWidth: 42, flexShrink: 0 }}>{r.rango}</div>
                      <div>
              ...
              <div style={{ marginTop: "1rem", padding: "1rem", background: "#EDF2F8", borderRadius: 10, border: "0.5px solid #C8D4E0" }}>
                <p style={{ fontSize: 13, color: "#1F3A5F", lineHeight: 1.6 }}><strong>Importante:</strong> estas puntuaciones son análisis independientes basados en fuentes públicas. No representan la posición oficial de la Iglesia Católica. El juicio final siempre es del lector.</p>
              </div>
            </div>
                ...
                {FAQS.map(function(faq, i) {
                  ...
                  return (
                    <div key={i} style={{ background: "#fff", border: "0.5px solid #C8D4E0", borderRadius: 10, overflow: "hidden" }}>
                      <button onClick={function() { setOpenFaq(open ? null : i); }} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem 1.25rem", background: "none", border: "none", cursor: "pointer", fontFamily: "'EB Garamond', Georgia, serif", fontSize: 16, color: "#1F3A5F", textAlign: "left", gap: 12 }}>
                        <span>{faq.q}</span>
                        <span style={{ fontSize: 12, color: "#8AAFD4", flexShrink: 0 }}>{open ? "▲" : "▼"}</span>
                      </button>
                      {open && <div style={{ padding: "0 1.25rem 1rem", fontSize: 13, color: "#6E6E73", lineHeight: 1.7 }}>{faq.a}</div>}
                    </div>
                  );
                })}
              </div>
            ...
            {!session && (
              <div style={{ background: "#1F3A5F", borderRadius: 14, padding: "2rem", textAlign: "center" }}>
                <h3 style={{ fontFamily: "'EB Garamond', Georgia, serif", fontSize: 26, fontWeight: 400, color: "#FAF7F0", marginBottom: "0.75rem" }}>¿Listo para unirte?</h3>
                <p style={{ fontSize: 13, color: "#8AAFD4", marginBottom: "1.25rem", lineHeight: 1.6 }}>Gratis. Sin contraseña. Solo tu cuenta de Google.</p>
                <button onClick={signInWithGoogle} style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "11px 24px", background: "#FAF7F0", color: "#1F3A5F", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "DM Sans, sans-serif" }}>
