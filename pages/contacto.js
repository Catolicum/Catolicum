...
export default function Contacto() {
  ...
  function handleChange(e) { setForm(Object.assign({}, form, { [e.target.name]: e.target.value })); }
  ...
  var inputStyle = { width: "100%", padding: "10px 14px", border: "0.5px solid #C8D4E0", borderRadius: 8, fontSize: 14, fontFamily: "DM Sans, sans-serif", color: "#1F2937", background: "#FFFFFF", boxSizing: "border-box" };
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ background: "#1F3A5F", borderBottom: "0.5px solid #2A4E7F", padding: isMobile ? "1.25rem 1.25rem 1rem" : "1.5rem 2rem 1.25rem", textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "'EB Garamond', Georgia, serif", fontSize: isMobile ? 14 : 15, fontStyle: "italic", color: "#E1B955", display: "inline-block", padding: "0 1rem", borderLeft: "2px solid #2A4E7F", borderRight: "2px solid #2A4E7F" }}>Sugerir un libro, reportar un error, o simplemente escribirnos</p>
          </div>
          <div style={{ maxWidth: 560, margin: "0 auto", padding: isMobile ? "1.25rem 1rem" : "2.5rem 1.5rem", width: "100%" }}>
            {sent ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                <div><label style={labelStyle}>Nombre</label><input type="text" name="nombre" required value={form.nombre} onChange={handleChange} placeholder="Tu nombre" style={{ ...inputStyle, height: 44 }} /></div>
                <div><label style={labelStyle}>Email</label><input type="email" name="email" required value={form.email} onChange={handleChange} placeholder="tu@email.com" style={{ ...inputStyle, height: 44 }} /></div>
                <div><label style={labelStyle}>Mensaje</label><textarea name="mensaje" required value={form.mensaje} onChange={handleChange} placeholder="Escribe tu mensaje aquí..." rows={5} style={{ ...inputStyle, resize: "vertical" }} /></div>
                <button type="submit" style={{ height: 44, background: "#1F3A5F", color: "#FAF7F0", border: "none", borderRadius: 8, fontSize: 14, fontFamily: "DM Sans, sans-serif", cursor: "pointer" }}>Enviar mensaje</button>
              </form>
            )}
