...
export default function Home() {
    ...
    if (!match) {
      supabase.from("libros").select("titulo, autor, puntuacion").eq("idioma", "es").gte("puntuacion", 7).limit(20).then(function(res) {
        if (res.data && res.data.length > 0) {
          setNotFoundSuggestions(res.data.sort(function() { return Math.random() - 0.5; }).slice(0, 4));
        }
      });
    }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FAF7F0", fontFamily: "DM Sans, sans-serif", color: "#1F2937" }}>
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position: "fixed", bottom: "1.5rem", left: "50%", transform: "translateX(-50%)", background: "#1F3A5F", color: "#FAF7F0", padding: "10px 20px", borderRadius: 20, fontSize: 13, zIndex: 1000, whiteSpace: "nowrap", boxShadow: "0 4px 16px rgba(0,0,0,0.2)" }}>
          {toast}
        </div>
      )}
      ...
      {showLoginPrompt && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: "1rem" }} onClick={function() { setShowLoginPrompt(false); }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "2rem 1.5rem", maxWidth: 340, width: "100%", textAlign: "center", boxShadow: "0 8px 40px rgba(0,0,0,0.15)" }} onClick={function(e) { e.stopPropagation(); }}>
            ...
            <button onClick={function() { setShowLoginPrompt(false); }} style={{ width: "100%", padding: 8, background: "none", border: "none", fontSize: 13, color: "#AEAEB2", cursor: "pointer", fontFamily: "DM Sans, sans-serif" }}>
              Ahora no
            </button>
          </div>
        </div>
          ...
          {isMobile && (
            <div style={{ background: "#1F3A5F", borderBottom: "0.5px solid #2A4E7F", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {user && wishlistSlugs.length > 0 && (
                  <span style={{ fontSize: 11, background: "#e8788a", color: "#fff", borderRadius: 10, padding: "2px 7px", fontWeight: 600 }}>♡ {wishlistSlugs.length}</span>
                )}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: "1rem" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, flexShrink: 0 }}>
                          ...
                          <HeartButton
                            ...
                            onNeedLogin={function() { setShowLoginPrompt(true); }}
                          />
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {notFoundSuggestions.map(function(b) {
                          ...
                          return (
                            <div key={b.titulo} style={{ display: "flex", alignItems: "center", gap: 10, padding: ".65rem .85rem", background: "#FAF7F0", border: "0.5px solid #E8E2D4", borderRadius: 8 }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={function() { setQuery(b.titulo); handleSearch(b.titulo); }}>
                                <div style={{ fontSize: 13, fontWeight: 500, color: "#1F3A5F", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.titulo}</div>
                ...
                <div style={{ maxWidth: 680, margin: "0 auto" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: ".4rem" }}>
                    {recomendados.map(function(b) {
                      ...
                      var rLibro = { slug: slug, titulo: b.titulo, autor: b.autor, puntuacion: b.puntuacion };
                      return (
                        <div key={b.titulo} style={{ display: "flex", alignItems: "center", gap: 12, background: "#fff", border: "0.5px solid #D8D0BC", borderRadius: 10, padding: ".75rem 1rem" }}>
                          <Link href={"/recomendados/" + slug} style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0, textDecoration: "none", color: "inherit" }}>
                            {b.imagen_url ? (
                          ...
                          <HeartButton libro={rLibro} user={user} isInWishlist={isInWishlist} onToggle={toggleWishlist} onNeedLogin={function() { setShowLoginPrompt(true); }} />
                        </div>
                      );
                    })}
                  </div>
